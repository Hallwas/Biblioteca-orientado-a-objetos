...
# Simulando uma data de devolução para 2 dias atrás (para forçar o sistema a gerar uma multa)
...
# Regra de Negócio 1: Bibliotecário tenta autorizar o empréstimo
autorizado = bibliotecario_joao.autorizar_emprestimo(contrato_emprestimo)
 
if autorizado:
    # Se ela autorizou, o livro sai da prateleira
    livro1.emprestar()
 
print("\nDevolução de multas")
 
# Regra de Negócio 2: Calculando multa na devolução
...
if valor_multa > 0:
    ...
    # Atualizando o cadastro do cliente com a dívida
...
# Finaliza o empréstimo (que chama automaticamente a devolução do item)
contrato_emprestimo.finalizar_emprestimo()
 
print("\nTestando a exibição de dados")
# Mostra que o livro voltou para a prateleira (Disponível)
livro1.exibir_dados()